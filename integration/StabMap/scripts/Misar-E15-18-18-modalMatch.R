...
read_data <- function(dir, name){
    ...
    cname = read.table(pj(dir, glue('{name}_cname.csv')), sep=',', header=T, row.names=1, comment.char="")
    feat_name = read.table(pj(dir, glue('{name}_feat.csv')), sep=',', header=T, comment.char="")
...
data_dir = '/disco_500t/xuhua/data/MISAR_seq/R_data_modalMatch/'
...
length(rownames(mult_rna_count))
length(unique(rownames(mult_rna_count)))
...
head(rownames(single_rna_count))

sum(duplicated(colnames(mult_rna_count)))
sum(duplicated(colnames(single_rna_count)))
...
write.csv(as.data.frame(stab_indirect), 
  file=paste0(out_dir, 'E15-18-18_modalMatch.csv'),
  quote=F, row.names=T)
